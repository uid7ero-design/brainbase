...
# (lib/data-hub/sourceMapping/{sourceSystems,sourceMappings,mappingVersions}.ts).
#
# Extends the exact disposable-container methodology established by
# scripts/tests/verify-worksheet-read-service.sh (5A.2H.2): a fresh
# postgres:16-alpine container, created and destroyed by this script only,
# never touching Production/Neon/Preview or any already-running database.
#
# WHAT THIS DOES: bootstraps organisations/users/uploads, applies the
# real scripts/create-import-batches.sql (5A.2C) to get import_batches
# into shape, then applies the real scripts/create-datahub-source-
# mappings.sql (5B.1 — the SAME file already certified and executed once
# against Production) to create source_systems/source_mappings/
# mapping_versions plus the nullable lineage columns, then runs
# `npx vitest run --config vitest.integration.config.ts
# scripts/tests/datahubSourceMappingServices.integration.test.ts` —
# exercising the REAL, unmodified production service functions against
# real Postgres constraint enforcement (tenant isolation, active-version
# pointer integrity, concurrent version-number allocation, activation
# atomicity), never a hand-duplicated copy of their Prisma calls.
#
# WHAT THIS DOES NOT DO: not wired into CI in this phase (same as every
# sibling Data Hub disposable-Postgres harness). Never connects to
# Neon/Production/Preview.
#
# USAGE:
#   bash scripts/tests/verify-datahub-source-mapping-services.sh
...
CONTAINER="datahub-5b2-service-harness-$$"
...
cd "$REPO_ROOT"
npx vitest run --config vitest.integration.config.ts scripts/tests/datahubSourceMappingServices.integration.test.ts
RESULT=$?

if [ $RESULT -eq 0 ]; then
  echo "PASS: Data Hub 5B.2 source mapping service integration suite."
else
  echo "FAIL: Data Hub 5B.2 source mapping service integration suite (exit $RESULT)."
fi
