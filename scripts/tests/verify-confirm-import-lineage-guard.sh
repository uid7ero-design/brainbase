...
# the legacy confirmImport() lineage guard (services/upload.ts).
#
# Extends the exact disposable-container methodology established by
# scripts/tests/verify-inspect-worksheets.sh: a fresh postgres:16-alpine
...
# (organisations/users/uploads — identical to verify-inspect-worksheets.sh's
# own bootstrap) plus the real scripts/create-import-batches.sql migration
# (which adds import_batches and the worksheet-lineage columns/constraints
# this guard depends on), then runs `npx vitest run --config
# vitest.integration.config.ts scripts/tests/confirmImportLineageGuard.integration.test.ts`
# — exercising the real, unmodified confirmImport() against real Postgres.
#
# WHAT THIS DOES NOT DO: it is not wired into CI in this phase (same as the
# sibling Data Hub harnesses). It never makes a live Vercel Blob network
# call and never connects to Neon/Production (DATABASE_URL always points at
# the disposable container this script itself creates).
#
# USAGE:
#   bash scripts/tests/verify-confirm-import-lineage-guard.sh

set -uo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
MIGRATION="$REPO_ROOT/scripts/create-import-batches.sql"
CONTAINER="datahub-5a2h3pre-confirm-import-guard-harness-$$"
...
cd "$REPO_ROOT"
npx vitest run --config vitest.integration.config.ts scripts/tests/confirmImportLineageGuard.integration.test.ts
RESULT=$?

if [ $RESULT -eq 0 ]; then
  echo "PASS: confirmImport lineage guard integration suite."
else
  echo "FAIL: confirmImport lineage guard integration suite (exit $RESULT)."
fi
