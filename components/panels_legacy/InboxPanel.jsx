...
export function InboxPanel() {
  const { inboxOpen, setInboxOpen } = useAppStore();

  const [gmailConnected, setGmailConnected] = useState(null);
  const [messages,  setMessages]  = useState([]);
  const [loading,   setLoading]   = useState(false);
  const [selected,  setSelected]  = useState(null); // full message object
  ...
  useEffect(() => {
    if (!inboxOpen) return;
    function onKey(e) { if (e.key === 'Escape') setInboxOpen(false); }
  ...
  return (
    ...
    >
      <div style={{ width: 'min(860px, 94vw)', height: 'min(680px, 88vh)', borderRadius: 16, border: '1px solid rgba(255,255,255,.10)', ...GLASS, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '14px 18px', borderBottom: '1px solid rgba(255,255,255,.07)', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          ...
          <button onClick={loadInbox} title="Refresh" style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', padding: 4, color: 'rgba(255,255,255,.52)', display: 'flex' }}>
