  ...
  return (
    <div style={{ padding: "12px 14px 14px", borderTop: "1px solid rgba(255,255,255,.05)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="rgba(0,207,234,.7)" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
        <span style={{ fontSize: 9, color: "rgba(255,255,255,.28)", letterSpacing: ".10em", fontWeight: 600 }}>AGENDA</span>
        ...
        <button onClick={() => { setRunning(false); setSeconds(mode === 'work' ? 25 * 60 : 5 * 60); }}
          style={{ background: "none", border: "none", cursor: "pointer", padding: 2, color: "rgba(255,255,255,.22)" }}>
        ...
        {['work', 'break'].map(m => (
          <button key={m} onClick={() => { setRunning(false); setMode(m); setSeconds(m === 'work' ? 25 * 60 : 5 * 60); }}
            style={{ flex: 1, padding: "3px 0", borderRadius: 5, border: "none", cursor: "pointer", fontSize: 8, fontWeight: 700, letterSpacing: ".06em",
              background: mode === m ? (m === 'work' ? "rgba(0,207,234,.10)" : "rgba(134,239,172,.10)") : "rgba(255,255,255,.03)",
        ...
        <div style={{ marginBottom: 7, display: "flex", flexDirection: "column", gap: 3 }}>
          {pending.slice(0, 6).map(task => (
            <button key={task.id} onClick={() => toggle(task.id)}
              ...
              <div style={{ width: 12, height: 12, borderRadius: 3, border: "1px solid rgba(0,207,234,.35)", flexShrink: 0, marginTop: 1 }} />
        ...
        <div style={{ marginBottom: 7 }}>
          {done.slice(-2).map(task => (
            <button key={task.id} onClick={() => toggle(task.id)}
              ...
              <span style={{ fontSize: 10, color: "rgba(255,255,255,.30)", lineHeight: 1.3, flex: 1, textDecoration: "line-through" }}>{task.text}</span>
            </button>
          ))}
          <button onClick={clearDone} style={{ fontSize: 9, color: "rgba(255,255,255,.20)", background: "none", border: "none", cursor: "pointer", padding: "2px 0" }}>
      ...
      <div style={{ display: "flex", gap: 5 }}>
        ...
        />
        <button onClick={addTask} style={{ padding: "5px 10px", borderRadius: 6, background: "rgba(0,207,234,.08)", border: "1px solid rgba(0,207,234,.20)", color: CYAN, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>+</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ padding: "12px 14px 14px", borderTop: "1px solid rgba(255,255,255,.05)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="rgba(0,207,234,.7)" strokeWidth="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
        <span style={{ fontSize: 9, color: "rgba(255,255,255,.28)", letterSpacing: ".10em", fontWeight: 600 }}>WORLD CLOCKS</span>
      </div>
  ...
  return (
    <div style={{ padding: "12px 14px 14px", borderTop: "1px solid rgba(255,255,255,.05)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
        ...
        <span style={{ fontSize: 9, color: "rgba(255,255,255,.28)", letterSpacing: ".10em", fontWeight: 600 }}>MEMORY</span>
        <button onClick={() => setTick(t => t + 1)} style={{ marginLeft: "auto", background: "none", border: "none", cursor: "pointer", padding: 2, color: "rgba(255,255,255,.22)" }}>
          <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-4.95"/></svg>
        </button>
      </div>
      {memories.length === 0 ? (
        <div style={{ fontSize: 10, color: "rgba(255,255,255,.20)", fontStyle: "italic" }}>No memories yet. Talk to Helena.</div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
          {memories.map((m, i) => (
            <div key={i} style={{ fontSize: 10, color: "rgba(255,255,255,.50)", lineHeight: 1.4, padding: "4px 7px", borderRadius: 5, background: "rgba(180,130,255,.05)", border: "1px solid rgba(180,130,255,.10)" }}>
              {m.fact}
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ padding: "12px 14px 14px", borderTop: "1px solid rgba(255,255,255,.05)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 12 }}>
        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="rgba(0,207,234,.7)" strokeWidth="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
        <span style={{ fontSize: 9, color: "rgba(255,255,255,.28)", letterSpacing: ".10em", fontWeight: 600 }}>METRICS</span>
      </div>
      {config.map(m => editId === m.id ? (
        <div key={m.id} style={{ marginBottom: 10, padding: "8px", borderRadius: 6, background: "rgba(0,207,234,.04)", border: "1px solid rgba(0,207,234,.15)" }}>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            <button onClick={saveEdit} style={{ flex: 1, padding: "3px 0", borderRadius: 4, background: "rgba(0,207,234,.10)", border: "1px solid rgba(0,207,234,.25)", color: CYAN, fontSize: 9, fontWeight: 600, cursor: "pointer" }}>Save</button>
            <button onClick={() => setEditId(null)} style={{ padding: "3px 8px", borderRadius: 4, background: "none", border: "1px solid rgba(255,255,255,.08)", color: "rgba(255,255,255,.25)", fontSize: 9, cursor: "pointer" }}>✕</button>
          </div>
        </div>
      ) : (
        <div key={m.id} style={{ display: "flex", alignItems: "center", marginBottom: 10, gap: 8 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 8, color: "rgba(255,255,255,.28)", letterSpacing: ".06em", fontWeight: 600, marginBottom: 1 }}>{m.label.toUpperCase()}</div>
            <div style={{ fontSize: 18, fontWeight: 700, color: "rgba(255,255,255,.80)", fontFamily: "monospace", letterSpacing: "-.02em", lineHeight: 1 }}>
              {values[m.id] ?? (m.url ? '…' : '--')}
            </div>
          </div>
          <button onClick={() => startEdit(m)} style={{ background: "none", border: "none", cursor: "pointer", padding: 3, color: "rgba(255,255,255,.18)", flexShrink: 0 }}>
...
// ── News Feed Widget ──────────────────────────────────────────────────────────
const CAT_DOT = { tech: "rgba(0,207,234,.8)", ai: "rgba(130,180,255,.8)", cyber: "rgba(255,120,80,.8)" };
...
function NewsWidget() {
  ...
  return (
    <div style={{ padding: "12px 14px 14px", borderTop: "1px solid rgba(255,255,255,.05)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="rgba(0,207,234,.7)" strokeWidth="2"><path d="M4 11a9 9 0 0 1 9 9"/><path d="M4 4a16 16 0 0 1 16 16"/><circle cx="5" cy="19" r="1"/></svg>
        <span style={{ fontSize: 9, color: "rgba(255,255,255,.28)", letterSpacing: ".10em", fontWeight: 600 }}>NEWS FEED</span>
        <button onClick={fetchNews} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, color: "rgba(255,255,255,.22)" }}>
          <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-4.95"/></svg>
        </button>
        <button onClick={() => setNewsOpen(true)} style={{ marginLeft: "auto", background: "none", border: "none", cursor: "pointer", padding: 2, color: "rgba(0,207,234,.45)", fontSize: 8, fontWeight: 600, letterSpacing: ".04em" }}>
          ALL →
        </button>
      </div>
      {loading ? (
        <div style={{ height: 28, display: "flex", alignItems: "center" }}>
          <div style={{ width: 4, height: 4, borderRadius: "50%", background: "rgba(0,207,234,.4)", animation: "agentPulse 1.2s ease-in-out infinite" }} />
        </div>
      ) : (
        <>
          <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
            {visible.map((a, i) => (
              <a key={a.id ?? i} href={a.url} target="_blank" rel="noopener noreferrer"
                ...
                <div className="nt" style={{ fontSize: 10, color: "rgba(255,255,255,.55)", lineHeight: 1.35, marginBottom: 2, transition: "color .15s" }}>
                  {a.title}
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                  <div style={{ width: 4, height: 4, borderRadius: "50%", background: CAT_DOT[a.category] ?? CAT_DOT.tech, flexShrink: 0 }} />
                  <span style={{ fontSize: 8, color: "rgba(255,255,255,.25)" }}>{a.source}</span>
          ...
          {articles.length > 4 && (
            <button onClick={() => setExpanded(e => !e)} style={{ marginTop: 7, background: "none", border: "none", cursor: "pointer", fontSize: 9, color: "rgba(255,255,255,.22)", padding: 0 }}>
...
function WeatherWidget() {
  ...
  if (loading) return (
    <div style={{ padding: "12px 14px", borderTop: "1px solid rgba(255,255,255,.05)", display: "flex", alignItems: "center", gap: 8 }}>
      <div style={{ width: 4, height: 4, borderRadius: "50%", background: "rgba(0,207,234,.4)", animation: "agentPulse 1.2s ease-in-out infinite" }} />
      <span style={{ fontSize: 10, color: "rgba(255,255,255,.22)" }}>Loading weather…</span>
    </div>
  );
  ...
  return (
    <div style={{ padding: "12px 14px 14px", borderTop: "1px solid rgba(255,255,255,.05)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="rgba(0,207,234,.7)" strokeWidth="2"><circle cx="12" cy="12" r="5"/><path d="M12 1v2M12 21v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M1 12h2M21 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42"/></svg>
        <span style={{ fontSize: 9, color: "rgba(255,255,255,.28)", letterSpacing: ".10em", fontWeight: 600 }}>SEAFORD RISE</span>
        <span style={{ fontSize: 9, color: "rgba(255,255,255,.18)", marginLeft: "auto" }}>SA</span>
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 10, color: "rgba(255,255,255,.40)", marginTop: 2 }}>{current.label}</div>
        </div>
        <div style={{ marginLeft: "auto", textAlign: "right" }}>
          <div style={{ fontSize: 10, color: "rgba(255,255,255,.35)" }}>Feels {current.feelsLike}°</div>
          <div style={{ fontSize: 10, color: "rgba(255,255,255,.28)", marginTop: 2 }}>💧 {current.humidity}%</div>
          <div style={{ fontSize: 10, color: "rgba(255,255,255,.28)", marginTop: 2 }}>💨 {current.wind} km/h</div>
        </div>
      </div>
      <div style={{ display: "flex", gap: 3, marginTop: 8 }}>
        {forecast.slice(0, 5).map((day, i) => (
          <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2, padding: "5px 2px", borderRadius: 6, background: i === 0 ? "rgba(0,207,234,.06)" : "rgba(255,255,255,.03)", border: i === 0 ? "1px solid rgba(0,207,234,.12)" : "1px solid transparent" }}>
            <span style={{ fontSize: 8, color: i === 0 ? "rgba(0,207,234,.8)" : "rgba(255,255,255,.30)", fontWeight: 600, letterSpacing: ".04em" }}>{i === 0 ? "NOW" : day.day.toUpperCase()}</span>
            ...
            <span style={{ fontSize: 8, color: "rgba(255,255,255,.30)" }}>{day.low}°</span>
...
function SpotifyWidget() {
  ...
  return (
    <div style={{ padding: "14px 14px 16px", borderTop: "1px solid rgba(255,255,255,.05)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
        {SPOTIFY_ICON}
        <span style={{ fontSize: 9, color: "rgba(255,255,255,.28)", letterSpacing: ".10em", fontWeight: 600 }}>NOW PLAYING</span>
        {connected && (
              ...
              {results.slice(0, 5).map(t => (
                <button key={t.id} onClick={() => handlePlay(t.uri)}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 9, color: "rgba(255,255,255,.35)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.artist}</div>
                  </div>
                </button>
              ))}
            </div>
          ) : query.trim() ? (
            <div style={{ fontSize: 10, color: "rgba(255,255,255,.22)", textAlign: "center", padding: "10px 0" }}>No results</div>
          ) : (
            <div style={{ fontSize: 10, color: "rgba(255,255,255,.18)", textAlign: "center", padding: "10px 0" }}>Type to search</div>
          )}
        </div>
      ) : (
        <>
  ...
  return (
    <div style={{ padding: "14px 14px 16px", borderTop: "1px solid rgba(255,255,255,.05)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
        ...
        <span style={{ fontSize: 9, color: "rgba(255,255,255,.28)", letterSpacing: ".10em", fontWeight: 600 }}>BRAIN</span>
      ...
      {editing ? (
        <div>
          ...
          <div style={{ display: "flex", gap: 5 }}>
            ...
            <button onClick={() => setEditing(false)} style={{ padding: "5px 8px", borderRadius: 6, background: "none", border: "1px solid rgba(255,255,255,.08)", color: "rgba(255,255,255,.30)", fontSize: 10, cursor: "pointer" }}>✕</button>
          </div>
        </div>
      ) : !hasVault ? (
        ...
        <>
          <div style={{ padding: "8px 10px", borderRadius: 7, background: "rgba(180,130,255,.06)", border: "1px solid rgba(180,130,255,.14)", marginBottom: 8 }}>
            ...
            <div style={{ fontSize: 9, color: "rgba(255,255,255,.30)" }}>
          ...
          <div style={{ display: "flex", gap: 5 }}>
            ...
            <button onClick={() => { setInput(status?.vaultPath ?? ''); setEditing(true); }} style={{ padding: "5px 8px", borderRadius: 6, background: "none", border: "1px solid rgba(255,255,255,.08)", color: "rgba(255,255,255,.30)", fontSize: 10, cursor: "pointer" }}>✎</button>
            <button onClick={forget} style={{ padding: "5px 8px", borderRadius: 6, background: "none", border: "1px solid rgba(255,255,255,.08)", color: "rgba(255,255,255,.25)", fontSize: 10, cursor: "pointer" }} title="Disconnect vault">✕</button>
          </div>
        </>
      )}
    </div>
...
export function LeftSidebar({ open, onToggle }) {
  ...
  const { activeNav, setActiveNav, setMemoryPanelOpen, setNewsOpen } = useAppStore();
  ...
  return (
    <div style={{ width: sidebarWidth, flexShrink: 0, transition: "width 280ms cubic-bezier(0.16,1,0.3,1)", display: "flex", flexDirection: "column", overflow: "hidden", position: "relative", borderRight: "1px solid rgba(255,255,255,.05)", background: "rgba(8,11,20,.50)", backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)" }}>
          ...
          <div style={{ display: "flex", gap: 3, padding: "12px 10px 0", paddingRight: 36, flexShrink: 0 }}>
            <button onClick={() => setActiveTab('nav')}
              style={{ flex: 1, padding: "5px 0", borderRadius: 6, border: "none", cursor: "pointer", fontSize: 9, fontWeight: 700, letterSpacing: ".08em", transition: "all .2s",
                background: activeTab === 'nav' ? "rgba(0,207,234,.10)" : "rgba(255,255,255,.03)",
                ...
                {NAV_ITEMS.map(item => {
                  ...
                  return (
                    <button key={item.label} onClick={() => handleNav(item)}
                      style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, padding: "7px 8px", borderRadius: 7, background: isActive ? "rgba(0,207,234,.08)" : "transparent", border: isActive ? "1px solid rgba(0,207,234,.14)" : "1px solid transparent", color: isActive ? CYAN : "rgba(255,255,255,.40)", fontSize: 11, fontWeight: isActive ? 600 : 400, cursor: "pointer", textAlign: "left", marginBottom: 2, transition: "all .2s" }}>
