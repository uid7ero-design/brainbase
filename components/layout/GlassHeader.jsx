...
import { GLASS, CYAN, VIOLET_SOFT } from "../../lib/utils/constants";
...
export function GlassHeader({ appState }) {
  ...
  const setContactsOpen    = useAppStore(s => s.setContactsOpen);
  ...
  return (
    <header style={{ height: 52, display: "flex", alignItems: "center", padding: "0 18px", gap: 12, flexShrink: 0, zIndex: 30, position: "relative", ...GLASS, borderBottom: "1px solid rgba(255,255,255,0.06)", borderTop: "none", borderLeft: "none", borderRight: "none" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1 }}>
        {/* Identity mark — swap BrandLogo internals when final SVG is ready */}
        <div style={{ width: 24, height: 24, borderRadius: 6, background: 'rgba(124,58,237,.14)', border: '1px solid rgba(124,58,237,.28)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        {/* HLNA status badge */}
        <div
          title="Hyper Learning Neural Agent"
          style={{ display: "flex", alignItems: "center", gap: 5, padding: "4px 10px", borderRadius: 20, background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.06)", cursor: "default" }}
        >
          <div style={{
            width: 6, height: 6, borderRadius: "50%",
            background: isListening ? CYAN : isActive ? VIOLET_SOFT : 'rgba(124,58,237,.40)',
            boxShadow: isListening ? `0 0 6px ${CYAN}` : isActive ? `0 0 6px ${VIOLET_SOFT}` : 'none',
            transition: "all .4s",
          }} />
          <span style={{
            fontSize: 10,
            color: isListening ? CYAN : isActive ? VIOLET_SOFT : "rgba(255,255,255,.40)",
            letterSpacing: ".08em", transition: "color .4s",
          }}>
            {isListening ? "HLNA LISTENING" : isActive ? "HLNA ACTIVE" : "HLNA ACTIVE"}
          </span>
        </div>
        ...
        <a
          ...
          title="Neural Map"
          style={{ width: 30, height: 30, borderRadius: 7, border: '1px solid rgba(124,58,237,.18)', background: 'rgba(124,58,237,.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', textDecoration: 'none' }}
        >
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="rgba(124,58,237,.70)" strokeWidth="1.8"><circle cx="12" cy="12" r="3"/><path d="M12 2v3M12 19v3M4.22 4.22l2.12 2.12M17.66 17.66l2.12 2.12M2 12h3M19 12h3M4.22 19.78l2.12-2.12M17.66 6.34l2.12-2.12"/></svg>
        </a>
        ...
        <button
          ...
          style={{ width: 30, height: 30, borderRadius: 7, border: '1px solid rgba(255,255,255,.06)', background: isFullscreen ? 'rgba(124,58,237,.08)' : 'rgba(255,255,255,.03)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
        >
          {isFullscreen
            ? <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="rgba(124,58,237,.75)" strokeWidth="1.8"><polyline points="8 3 3 3 3 8"/><polyline points="21 8 21 3 16 3"/><polyline points="3 16 3 21 8 21"/><polyline points="16 21 21 21 21 16"/></svg>
            : <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,.45)" strokeWidth="1.8"><polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/></svg>}
        </button>
        ...
        <button
          ...
          title="Address Book"
          style={{ width: 30, height: 30, borderRadius: 7, border: '1px solid rgba(255,255,255,.06)', background: 'rgba(255,255,255,.03)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
        >
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,.40)" strokeWidth="1.8"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
        </button>
