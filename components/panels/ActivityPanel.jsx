...
export function ActivityPanel({ items, latestId, open, onToggle }) {
  return (
    <div style={{ width: open ? 320 : 36, flexShrink: 0, transition: "width 280ms cubic-bezier(0.16,1,0.3,1)", background: "rgba(8,11,20,.50)", backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)", borderLeft: "1px solid rgba(255,255,255,.05)", display: "flex", flexDirection: "column", overflow: "hidden", position: "relative" }}>
      ...
      {open && (
        <>
          {/* Scrollable agents + activity feed */}
          <div style={{ flex: 1, overflowY: "auto", paddingTop: 10 }}>

            {/* Agents section */}
            <div style={{ padding: "4px 14px 8px" }}>
              <span style={{ fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,.46)", letterSpacing: ".12em" }}>AGENTS</span>
            </div>
              ...
              {AGENTS.map(agent => (
                <div key={agent.id} style={{ padding: "9px 10px", borderRadius: 8, background: "rgba(255,255,255,.025)", border: "1px solid rgba(255,255,255,.04)", cursor: "pointer", transition: "all .2s" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 3 }}>
                    ...
                    <span style={{ fontSize: 9, padding: "1px 5px", borderRadius: 3, background: "rgba(255,255,255,.06)", color: "rgba(255,255,255,.52)" }}>{agent.tasks}</span>
                  </div>
                  <div style={{ fontSize: 9, color: "rgba(255,255,255,.50)", lineHeight: 1.4, paddingLeft: 14 }}>{agent.last}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{ padding: "0 14px 8px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <span style={{ fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,.46)", letterSpacing: ".12em" }}>ACTIVITY</span>
              <span style={{ fontSize: 9, color: "rgba(255,255,255,.42)", letterSpacing: ".08em" }}>{items.length} TOTAL</span>
            </div>
              ...
              return (
                <div key={sec.type} style={{ marginBottom: 4 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 14px 6px" }}>
                    ...
                    <span style={{ fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,.46)", letterSpacing: ".12em", flex: 1 }}>{sec.label}</span>
                    <span style={{ fontSize: 9, padding: "1px 5px", borderRadius: 3, background: "rgba(255,255,255,.05)", color: "rgba(255,255,255,.50)" }}>{secItems.length}</span>
                  </div>
                        ...
                        <div key={item.id} style={{ padding: "9px 10px", borderRadius: 8, background: isNew ? "rgba(0,207,234,.06)" : "rgba(255,255,255,.025)", border: `1px solid ${isNew ? "rgba(0,207,234,.18)" : "rgba(255,255,255,.04)"}`, animation: "slideInRight .3s cubic-bezier(0.16,1,0.3,1) both", cursor: "pointer", transition: "all .3s" }}>
                          <div style={{ display: "flex", alignItems: "flex-start", gap: 6 }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              {item.sub && <div style={{ fontSize: 10, color: "rgba(255,255,255,.52)", lineHeight: 1.35, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.sub}</div>}
                            ...
                            <span style={{ fontSize: 9, color: "rgba(255,255,255,.38)", flexShrink: 0, marginTop: 1 }}>{item.time}</span>
                          </div>
                          {isNew && (
          ...
          {/* Brain section — pinned to bottom */}
          <div style={{ flexShrink: 0, borderTop: "1px solid rgba(255,255,255,.05)" }}>
            <BrainWidget />
            <NeuralMapCard style={{
              position: 'relative', bottom: 'auto', right: 'auto',
              width: '100%', height: 160, zIndex: 'auto',
              borderRadius: 0, border: 'none',
              borderTop: '1px solid rgba(180,130,255,.12)',
            }} />
          </div>
        </>
      )}
