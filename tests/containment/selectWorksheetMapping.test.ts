...
describe("selectWorksheetMapping — no Phase 6 / recognition / scheduling leakage", () => {
  // Data Hub 6.1B — the route (never the service) now carries exactly one
  // deliberate, authorized reference: DUPLICATE_SOURCE_EXTERNAL_ID_IN_WORKSHEET,
  // one entry in its exhaustive FailureCode->status Record (TypeScript
  // requires every route with such a Record to enumerate every code, even
  // ones it can never actually return — see the existing, identical
  // SOURCE_ALREADY_IMPORTED/MAPPING_COMPILE_FAILED precedent immediately
  // above it). This is not new reconciliation BEHAVIOR reaching this
  // route — selectWorksheetMapping itself is untouched (see the dedicated
  // byte-identity/no-selectWorksheetMapping-reference tests elsewhere in
  // this file) — only a type-exhaustiveness requirement satisfied with an
  // unreachable-from-here status mapping, exactly like its predecessors.
  it("service and route contain no Phase 6 (reconciliation/Observation/SourceRecordIdentity), recognition, or lib/integrations reference — 'external_id' is scoped to exclude the one authorized 6.1B exhaustive-mapping entry", () => {
    for (const file of [SERVICE_PATH, ROUTE_PATH]) {
      const code = stripComments(read(file));
      const withoutAuthorizedException = code.replace(/DUPLICATE_SOURCE_EXTERNAL_ID_IN_WORKSHEET/g, "");
