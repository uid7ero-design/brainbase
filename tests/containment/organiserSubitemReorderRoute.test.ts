...
const route = await import('@/app/api/organiser/boards/[boardId]/items/[parentItemId]/reorder/route')
...
const CTX = { params: Promise.resolve({ boardId: BOARD, parentItemId: PARENT }) }
...
beforeEach(() => {
  ...
  sqlCalls = []
  sqlResults = [[{ id: BOARD }], [{ id: PARENT }]]
  requireSessionMock.mockResolvedValue(SESSION)
  requireCapabilityMock.mockResolvedValue({ key: 'organiser', config: {} })
})

describe('POST /api/organiser/boards/[boardId]/items/[parentItemId]/reorder', () => {
  it('rejects malformed parent ids before SQL or the primitive', async () => {
    const res = await route.POST(jsonReq({ ordered_item_ids: [ITEM_A] }), { params: Promise.resolve({ boardId: BOARD, parentItemId: 'bad' }) })
  ...
  it('board not found -> 404 and primitive is never called', async () => {
    sqlResults = [[]]
    const res = await route.POST(jsonReq({ ordered_item_ids: [ITEM_A] }), CTX)
    expect(res.status).toBe(404)
    expect(resequenceMock).not.toHaveBeenCalled()
  })

  it('validates the parent inside the same organisation and board', async () => {
    sqlResults = [[{ id: BOARD }], []]
    const res = await route.POST(jsonReq({ ordered_item_ids: [ITEM_A] }), CTX)
    expect(res.status).toBe(404)
    expect(resequenceMock).not.toHaveBeenCalled()
    ...
    expect(sqlCalls[1].values).toContain(PARENT)
  })

  it('rejects missing or non-string ordered_item_ids before the primitive', async () => {
    const missing = await route.POST(jsonReq({}), CTX)
    expect(missing.status).toBe(400)
    expect(resequenceMock).not.toHaveBeenCalled()

    sqlResults = [[{ id: BOARD }], [{ id: PARENT }]]
    const mixed = await route.POST(jsonReq({ ordered_item_ids: [ITEM_A, 42] }), CTX)
    expect(mixed.status).toBe(400)
    expect(resequenceMock).not.toHaveBeenCalled()
  })

  it('passes only session-derived organisation, board, parent and complete order to the E1 primitive', async () => {
    resequenceMock.mockResolvedValue({ ok: true, order: [] })
    await route.POST(jsonReq({ ordered_item_ids: [ITEM_B, ITEM_A], organisation_id: 'evil', parent_item_id: 'evil' }), CTX)
    expect(resequenceMock).toHaveBeenCalledWith(
      { type: 'subitems', organisationId: 'org-a', boardId: BOARD, parentItemId: PARENT },
      [ITEM_B, ITEM_A],
    )
  })
  ...
  it('success -> 200 with authoritative order', async () => {
    ...
    const res = await route.POST(jsonReq({ ordered_item_ids: [ITEM_B, ITEM_A] }), CTX)
  ...
  it('never writes an activity row itself', async () => {
    ...
    const src = fs.readFileSync(path.resolve(__dirname, '../../app/api/organiser/boards/[boardId]/items/[parentItemId]/reorder/route.ts'), 'utf8')
