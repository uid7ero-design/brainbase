/**
 * Waste / Operations calculations — extracted from legacy ops-dashboard-kit.
 * Source data: service_requests table (was ops_events.csv).
 *
 * All functions are pure — no DB access, no side effects.
 * Input: raw rows from the DB query layer.
 */
...
export function computeOpsKpi(rows: OpsRow[]): OpsKpi {
  if (rows.length === 0) {
    return {
      total_issues: 0, open: 0, resolved: 0,
      repeat_rate: 0, avg_response_hours: 0, complaint_count: 0,
      resolution_rate: 0, top_hotspots: [],
    };
  }
  ...
  const responseTimes = rows
    .filter(r => r.response_hours != null && r.response_hours > 0)
  ...
  return {
    ...
    open,
    resolved,
    repeat_rate:      rows.length > 0 ? Math.round((repeats / rows.length) * 1000) / 10 : 0,
    avg_response_hours,
    complaint_count,
    resolution_rate:  rows.length > 0 ? Math.round((resolved / rows.length) * 1000) / 10 : 0,
...
export function aggregateByWeek(rows: OpsRow[]): WeeklyKpi[] {
  ...
  return Array.from(byWeek.entries())
    ...
    .map(([week_start, wRows]) => {
      const kpi = computeOpsKpi(wRows);
      return {
        week_start,
        total_issues:       wRows.length,
        open_issues:        kpi.open,
        in_progress:        wRows.filter(r => r.status.toLowerCase() === "in_progress" || r.status.toLowerCase() === "in progress").length,
...
export function computeSuburbHotspots(rows: OpsRow[]): SuburbHotspot[] {
  ...
  for (const row of rows) {
    ...
    const entry = map.get(suburb)!;
    entry.count++;
    if (row.repeat_issue) entry.repeat++;
    entry.complaints += row.complaint_count ?? 0;
    entry.severity_total += severityScore(row.severity);
  }
  ...
  return Array.from(map.entries())
    .map(([suburb, d]) => ({
      suburb,
      count:          d.count,
      repeat_count:   d.repeat,
      complaint_count: d.complaints,
      severity_score: Math.round((d.severity_total / d.count) * 10) / 10,
    }))
...
export function computeZoneBreakdown(
  ...
  const byZonePrev = new Map<string, number>();

  for (const r of rows)     { const z = r.zone ?? "Unknown"; byZone.set(z, (byZone.get(z) ?? 0) + 1); }
  for (const r of prevRows) { const z = r.zone ?? "Unknown"; byZonePrev.set(z, (byZonePrev.get(z) ?? 0) + 1); }

  return Array.from(byZone.entries())
    .map(([zone, count]) => {
      const prev = byZonePrev.get(zone) ?? 0;
...
export function weekOnWeekDelta(current: WeeklyKpi, prior: WeeklyKpi | undefined): {
  total_delta: number;
  repeat_delta: number;
  complaint_delta: number;
  response_delta: number;
} {
  if (!prior) return { total_delta: 0, repeat_delta: 0, complaint_delta: 0, response_delta: 0 };
  return {
    total_delta:     current.total_issues   - prior.total_issues,
    repeat_delta:    current.repeat_issue_count - prior.repeat_issue_count,
    complaint_delta: current.complaint_count   - prior.complaint_count,
    response_delta:  Math.round((current.avg_response_hours - prior.avg_response_hours) * 10) / 10,
  };
}
