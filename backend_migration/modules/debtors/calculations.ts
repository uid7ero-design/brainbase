/**
 * Debtor calculations — aging analysis, recovery rates, risk segmentation.
 */
...
export function computeDebtorKpi(rows: DebtorRow[]): DebtorKpi {
  ...
  // Recovery rate = (original - outstanding) / original across accounts with original_amount
  const withOriginal = rows.filter(r => r.original_amount != null && r.original_amount > 0);
  const recovery_rate = withOriginal.length > 0
    ? Math.round(
        withOriginal.reduce((sum, r) =>
          sum + (((r.original_amount! - r.outstanding_amount) / r.original_amount!) * 100), 0
        ) / withOriginal.length * 10
      ) / 10
    : 0;

  const at_risk_amount = rows
    .filter(r => r.days_overdue > 90 || r.aging_bucket === "DAYS_90_PLUS")
...
export function debtorRiskSegments(rows: DebtorRow[]): {
  high_risk: DebtorRow[];
  medium_risk: DebtorRow[];
  low_risk: DebtorRow[];
} {
  return {
    high_risk:   rows.filter(r => r.days_overdue > 90 || r.outstanding_amount > 10000),
    medium_risk: rows.filter(r => r.days_overdue > 30 && r.days_overdue <= 90),
    low_risk:    rows.filter(r => r.days_overdue <= 30),
  };
}
