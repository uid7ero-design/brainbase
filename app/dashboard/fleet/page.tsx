...
type Asset       = { id:string;type:string;make:string;year:number;department:string;driver:string;km:number;wages:number;fuel:number;maintenance:number;rego:number;repairs:number;insurance:number;depreciation:number;services:number;defects:number;total:number;totalWithDepr:number;costPerKm:number };
...
function HRTab({data}:{data:HRRecord[]}) {
  ...
  return(
    <><div className="grid grid-cols-4 gap-4 mb-8">
      ...
      <KPI2 icon={<Clock size={16}/>} label="Total Hours Worked" value={fmtH(totalWorked)} accent="emerald"/>
      <KPI2 icon={<Activity size={16}/>} label="Overtime Hours" value={fmtH(totalOT)} sub={`${((totalOT/totalWorked)*100).toFixed(1)}% of total`} accent="amber"/>
...
function DowntimeTab({data}:{data:DtRecord[]}) {
  ...
  const byAsset=Object.entries(data.reduce<Record<string,number>>((a,r)=>({...a,[r.asset]:(a[r.asset]||0)+r.hours}),{})).map(([id,hours])=>({id,hours})).sort((a,b)=>b.hours-a.hours);
  ...
  return(
    <><div className="grid grid-cols-4 gap-4 mb-8">
      <KPI2 icon={<Clock size={16}/>} label="Total Downtime" value={fmtH(totalHrs)} accent="red"/>
      <KPI2 icon={<DollarSign size={16}/>} label="Downtime Cost" value={fmt(totalCost)} accent="blue"/>
      <KPI2 icon={<AlertCircle size={16}/>} label="Total Incidents" value={String(data.length)} accent="amber"/>
...
function UtilisationTab({data}:{data:UtilRecord[]}) {
  if (!data.length) return <Empty/>;
  const avgUtil=data.reduce((s,r)=>s+r.utilisationPct,0)/data.length;
  const totalIdle=data.reduce((s,r)=>s+r.idleHours,0);
  const totalIdleCost=data.reduce((s,r)=>s+r.idleCost,0);
  const best=[...data].sort((a,b)=>b.utilisationPct-a.utilisationPct)[0];
  const chart=data.map(r=>({id:r.asset,operating:r.operatingHours,idle:r.idleHours,pct:r.utilisationPct}));
  return(
    <><div className="grid grid-cols-4 gap-4 mb-8">
      <KPI2 icon={<Gauge size={16}/>} label="Avg Utilisation" value={`${avgUtil.toFixed(1)}%`} accent="blue"/>
      <KPI2 icon={<Clock size={16}/>} label="Total Idle Hours" value={fmtH(totalIdle)} accent="amber"/>
      <KPI2 icon={<DollarSign size={16}/>} label="Total Idle Cost" value={fmt(totalIdleCost)} accent="red"/>
...
function GeofenceTab({trips,stops,coloc}:{trips:TripRecord[];stops:StopRecord[];coloc:ColocRecord[]}) {
  ...
  const assets=['All',...Array.from(new Set(trips.map(t=>t.asset))).sort()];
  const dates=['All',...Array.from(new Set(trips.map(t=>t.date)))];
  const filteredTrips=trips.filter(t=>assetFilter==='All'||t.asset===assetFilter).filter(t=>dateFilter==='All'||t.date===dateFilter);
  const filteredStops=stops.filter(s=>assetFilter==='All'||s.asset===assetFilter).filter(s=>dateFilter==='All'||s.date===dateFilter);
  const totalTrips=filteredTrips.length;
  const avgHoursOut=totalTrips>0?+(filteredTrips.reduce((s,t)=>s+t.hoursOut,0)/totalTrips).toFixed(1):0;
  const totalStops=filteredTrips.reduce((s,t)=>s+t.stopsMade,0);
  const colocEvents=coloc.filter(c=>dateFilter==='All'||c.date===dateFilter).length;
  const hoursChart=Array.from(new Set(trips.map(t=>t.asset))).map(a=>{const rows=filteredTrips.filter(t=>t.asset===a);return{id:a,avgHrs:rows.length?+(rows.reduce((s,t)=>s+t.hoursOut,0)/rows.length).toFixed(1):0};}).filter(d=>d.avgHrs>0);
  const stopTypePie=Object.entries(filteredStops.reduce<Record<string,number>>((a,s)=>({...a,[s.stopType]:(a[s.stopType]||0)+s.durationMins}),{})).map(([name,value])=>({name,value})).sort((a,b)=>b.value-a.value);
  const dwellChart=Object.entries(filteredStops.reduce<Record<string,number>>((a,s)=>({...a,[s.area]:(a[s.area]||0)+s.durationMins}),{})).map(([area,mins])=>({area:area.replace(' / Southern Suburbs','').replace(' / Southern',''),mins,hrs:+(mins/60).toFixed(1)})).sort((a,b)=>b.mins-a.mins).slice(0,8);
  const filtColoc=coloc.filter(c=>dateFilter==='All'||c.date===dateFilter);
  return(
    <><div className="grid grid-cols-4 gap-4 mb-6">
      <KPI2 icon={<Truck size={16}/>} label="Total Trips" value={String(totalTrips)} accent="blue"/>
      <KPI2 icon={<Clock size={16}/>} label="Avg Hours Out" value={`${avgHoursOut} hrs`} accent="emerald"/>
...
export default function FleetDashboard() {
  ...
  const overviewContent = assets.length === 0 ? (
    ...
    <>
      ...
      {/* ── Second row: dept chart + assets to watch ── */}
            ...
            <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 20 }}>
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={LIFECYCLE_SAMPLE}>
                  ...
                  <Tooltip formatter={(v:any)=>`$${Number(v).toLocaleString()}`} contentStyle={{ background: '#1a1a2e', border: 'none', borderRadius: 8 }} />
              ...
              <tbody>{LIFECYCLE_SAMPLE.map((a,i) => {
                ...
                return (
                  <tr key={a.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', background: i%2===0?'transparent':'rgba(255,255,255,0.02)' }}>
                    ...
                    <td style={{ padding: '11px 14px', color: 'rgba(255,255,255,0.6)' }}>{a.type}</td>
                    <td style={{ padding: '11px 14px' }}>{a.year}</td>
                    <td style={{ padding: '11px 14px', color: age >= 8 ? '#f87171' : age >= 6 ? '#fbbf24' : '#4ade80', fontWeight: 600 }}>{age}</td>
                    <td style={{ padding: '11px 14px' }}>${a.totalOwnership.toLocaleString()}</td>
                    <td style={{ padding: '11px 14px', color: a.costPerKm > 3 ? '#f87171' : a.costPerKm > 2 ? '#fbbf24' : a.costPerKm > 0 ? '#4ade80' : 'rgba(255,255,255,0.4)' }}>{a.costPerKm > 0 ? `$${a.costPerKm}` : '—'}</td>
                    <td style={{ padding: '11px 14px' }}>${a.maintenance.toLocaleString()}</td>
