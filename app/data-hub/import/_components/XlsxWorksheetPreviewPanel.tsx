...
export default function XlsxWorksheetPreviewPanel({ state, onBack, onRetry, onRestart }: {
  ...
  return <div>
    <h2 style={{ fontSize: 16, fontWeight: 600, color: "#f9fafb" }}>{state.worksheet.worksheetName}</h2>
    <p style={{ fontSize: 12, color: "rgba(249,250,251,.6)" }}>Worksheet {position} of {state.worksheets.length}</p>
    ...
    {preview && <>
      <p style={{ fontSize: 12, color: "rgba(249,250,251,.7)" }}>{preview.rowCount} rows · {preview.columnCount} columns</p>
      <div tabIndex={0} style={{ overflowX: "auto", border: "1px solid rgba(255,255,255,.08)", borderRadius: 8 }}>
        <table style={{ borderCollapse: "collapse", width: "100%", fontSize: 12 }}>
          <thead><tr>{Array.from({ length: visibleColumnCount }, (_, index) => <th key={index} scope="col" style={{ textAlign: "left", padding: 8, borderBottom: "1px solid rgba(255,255,255,.1)" }}>{preview.headers[index] || `Column ${index + 1}`}</th>)}</tr></thead>
          <tbody>{preview.sampleRows.map((row, rowIndex) => <tr key={rowIndex}>{Array.from({ length: visibleColumnCount }, (_, columnIndex) => <td key={columnIndex} style={{ padding: 8, borderBottom: "1px solid rgba(255,255,255,.05)" }}>{row[columnIndex] ?? ""}</td>)}</tr>)}</tbody>
        </table>
      </div>
