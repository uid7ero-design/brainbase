...
export type CheckoutSuccessClientProps = {
  ...
  eventSlug: string;
  // Both resolved server-side (page.tsx) via the same resolvePublicEvent
  // choke point every other public Events surface uses — never derived
  // from the URL slug on this client. organisationName is null only in
  // the rare case that server-side resolution itself failed (e.g. the
  // event was unpublished in the moments between a successful payment
  // and this redirect); see the render logic below for how that
  // degrades to unbranded chrome rather than ever inventing a name.
...
export default function CheckoutSuccessClient({ organisationSlug, eventSlug, organisationName, branding }: CheckoutSuccessClientProps) {
  const searchParams = useSearchParams();
  const sessionId = searchParams.get('session_id');
  // Public event STRUCTURAL presentation — same resolver, same
  // organisationSlug-only lookup, as PublicEventClient. Slug-keyed
  // lookup here is STRUCTURAL (variant/palette) only, never identity —
  // organisation identity (name/logo/website/accent) comes exclusively
  // from the branding/organisationName props above, resolved
  // server-side from the trusted organisationId, matching every other
  // public Events surface.
  const theme = resolvePublicEventTheme(organisationSlug);
  const institutional = theme.variant === 'institutional';
  const effectiveTokens = applyAccentOverride(theme.tokens, branding?.accentColor ?? null);
  const cssVars = cssVarsFor(effectiveTokens);
  // Only render the branded institutional chrome when server-side
  // resolution actually succeeded (organisationName !== null). If it
  // didn't, this page falls back to the same plain BrainBase header the
  // default variant already uses — a temporarily-unbranded degradation,
  // never a slug-derived identity.
  const hasResolvedIdentity = organisationName !== null;
    ...
    async function poll() {
      try {
        const res = await fetch(
          `/api/public/events/${organisationSlug}/${eventSlug}/checkout/status?session_id=${encodeURIComponent(sessionId as string)}`,
        );
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, color: TEXT_PRIMARY, fontFamily: FONT, display: 'flex', flexDirection: 'column', ...cssVars }}>
      {institutional && hasResolvedIdentity ? (
      ...
      {institutional && hasResolvedIdentity && <InstitutionalFooter branding={branding} organisationName={organisationName} />}
    </div>
  );
}
