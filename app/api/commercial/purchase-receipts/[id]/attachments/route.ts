...
import { listAttachmentsForPurchaseReceipt, uploadPurchaseReceiptAttachment, MAX_ATTACHMENT_BYTES } from '@/lib/commercial/documentAttachments';

type Ctx = { params: Promise<{ id: string }> };

// Phase C7.3 — Supporting Documents for a purchase receipt. Mirrors
// app/api/commercial/purchase-orders/[id]/attachments/route.ts exactly.
// GET: view/download listing, available in every status. POST: upload —
// DRAFT only, matching isPurchaseReceiptEditable()'s own single
// DRAFT-only rule.
export async function GET(_req: NextRequest, { params }: Ctx) {
  ...
  const attachments = await listAttachmentsForPurchaseReceipt(auth.session.organisationId, id);
...
export async function POST(req: NextRequest, { params }: Ctx) {
  ...
  if (!isPurchaseReceiptEditable(purchaseReceipt.status)) {
    return NextResponse.json({ error: `Purchase receipt is ${purchaseReceipt.status} — supporting documents can only be added while it is DRAFT.` }, { status: 409 });
  }
  ...
  const result = await uploadPurchaseReceiptAttachment({
