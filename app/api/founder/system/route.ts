...
// Microsoft 365 (Phase E.5A) connection state. Per the Phase E.0 audit
// and a read-only Production check James ran directly against Neon:
// integrations, sync_jobs, and agent_runs do not exist in Production,
// and users.last_login_at is populated for 0 of 5 users — none of those
// are queried anywhere in this route or in lib/founder/systemSignals.ts.
// No secrets, tokens, or integration config are ever included in the
// response.
...
export async function GET() {
  ...
  const [database, instagram, microsoft365] = await Promise.all([
    checkDatabase(),
    getInstagramState(),
    getMicrosoftState(),
  ]);
  ...
  return NextResponse.json({
    ...
    services: {
      ...
      microsoft365: { state: microsoft365 },
    },
  });
}
