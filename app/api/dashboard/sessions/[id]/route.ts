...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const sessions = await sql`
      SELECT id, name, day_of_week, start_time, duration_minutes, max_capacity, session_type, resource_id, recurring, created_at
      FROM sessions
    ...
    try {
      instances = await sql`
        SELECT si.id, si.date, si.start_time, si.duration_minutes, si.max_capacity, si.status, si.created_at,
          COALESCE(
      ...
      instances = await sql`
        SELECT id, date, start_time, duration_minutes, max_capacity, status, created_at
        FROM session_instances WHERE session_id = ${id} ORDER BY date ASC
      `
      instances = instances.map((i: Record<string, unknown>) => ({ ...i, enrolled_count: 0 }))
    }
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params
  let body: Partial<{ name: string; day_of_week: number; start_time: string; duration_minutes: number; max_capacity: number; session_type: string; resource_id: string | null; recurring: boolean }>
  ...
  try {
    const rows = await sql`
      ...
      WHERE id = ${id} AND organisation_id = ${session.organisationId}
      RETURNING id, name, day_of_week, start_time, duration_minutes, max_capacity, session_type, resource_id, recurring, created_at
    `
