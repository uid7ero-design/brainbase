      ...
      <form onSubmit={submit} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div>
          ...
          {!loadingPOs && purchaseOrders.length === 0 && (
            <p style={{ fontSize: 12, color: 'var(--text-secondary)', margin: '6px 0 0' }}>No issued purchase orders are available to bill against yet.</p>
          )}
        </div>
        ...
        <div>
          ...
          <p style={{ fontSize: 11, color: 'var(--text-secondary)', margin: '6px 0 0' }}>Distinct from the internal bill number BrainBase allocates when this bill is posted. Recording the same supplier invoice number twice for this supplier is rejected.</p>
        </div>
