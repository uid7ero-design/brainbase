    ...
    <div style={{ maxWidth: 1100 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: 13, margin: '4px 0 0' }}>{quotes.length} total</p>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <Link href="/commercial/quotes/new" style={btn('var(--purple-600)')}>+ New Quote</Link>
        </div>
      </div>
            ...
            {filtered.map((q, i) => (
              <tr key={q.id} style={{ borderBottom: i < filtered.length - 1 ? `1px solid ${BORDER}` : 'none' }}>
                <td style={{ padding: '13px 16px' }}>
                  <Link href={`/commercial/quotes/${q.id}`} style={{ color: 'var(--text-primary)', textDecoration: 'none', fontWeight: 500, fontSize: 14 }}>
                ...
                <td style={{ padding: '13px 16px' }}>
                  <Link href={`/commercial/quotes/${q.id}`} style={{ fontSize: 12, color: 'var(--text-secondary)', textDecoration: 'none' }}>View →</Link>
                </td>
              </tr>
            ))}
          </tbody>
