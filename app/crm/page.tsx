  ...
  return (
    <div style={{ maxWidth: 1000 }}>
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ color: 'var(--text-secondary)', fontSize: 13, margin: '4px 0 0' }}>Companies, contacts, deals & activities</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12, marginBottom: 28 }}>
        {[
          ...
          >
            ...
            <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 4, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
              {s.label}
            </div>
          </Link>
        ))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', borderBottom: `1px solid ${BORDER}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Link href="/crm/deals" style={{ fontSize: 12, color: 'var(--text-secondary)', textDecoration: 'none' }}>View all →</Link>
          </div>
          {loading ? (
            <p style={{ padding: 20, color: 'var(--text-muted)', fontSize: 13 }}>Loading…</p>
          ) : pipeline.length === 0 ? (
            <p style={{ padding: 20, color: 'var(--text-muted)', fontSize: 13 }}>
              No open deals. <Link href="/crm/deals" style={{ color: 'var(--purple-600)', textDecoration: 'none' }}>Add one →</Link>
            </p>
          ) : (
            pipeline.slice(0, 6).map((d, i) => (
              ...
              >
                <div>
                  ...
                  {d.company_name && <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 2 }}>{d.company_name}</div>}
                </div>
                <div style={{ textAlign: 'right' }}>
                  {d.value != null && <div style={{ fontSize: 13, fontWeight: 600, color: STAGE_COLORS[d.stage] }}>${Number(d.value).toLocaleString()}</div>}
                  <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2, textTransform: 'capitalize' }}>{d.stage.replace('_', ' ')}</div>
                </div>
              </Link>
            ))
          )}
        ...
        <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', borderBottom: `1px solid ${BORDER}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Link href="/crm/activities" style={{ fontSize: 12, color: 'var(--text-secondary)', textDecoration: 'none' }}>View all →</Link>
          </div>
          {loading ? (
            <p style={{ padding: 20, color: 'var(--text-muted)', fontSize: 13 }}>Loading…</p>
          ) : activities.length === 0 ? (
            <p style={{ padding: 20, color: 'var(--text-muted)', fontSize: 13 }}>No activity yet.</p>
          ) : (
