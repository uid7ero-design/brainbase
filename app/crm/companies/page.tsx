    ...
    <div style={{ maxWidth: 1000 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: 13, margin: '4px 0 0' }}>{companies.length} total</p>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          />
          <button onClick={() => setShowAdd(true)} style={btn('var(--purple-600)')}>+ Add Company</button>
        </div>
      </div>
            ...
            {filtered.map((c, i) => (
              <tr key={c.id} style={{ borderBottom: i < filtered.length - 1 ? `1px solid ${BORDER}` : 'none' }}>
                <td style={{ padding: '13px 16px' }}>
                  <Link href={`/crm/companies/${c.id}`} style={{ color: 'var(--text-primary)', textDecoration: 'none', fontWeight: 500, fontSize: 14 }}>{c.name}</Link>
                  {c.website && <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>{c.website}</div>}
                </td>
                ...
                <td style={{ padding: '13px 16px' }}>
                  <Link href={`/crm/companies/${c.id}`} style={{ fontSize: 12, color: 'var(--text-secondary)', textDecoration: 'none' }}>View →</Link>
                </td>
              </tr>
            ))}
          </tbody>
