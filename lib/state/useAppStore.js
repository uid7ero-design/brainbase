import { create } from 'zustand';
import { persist } from 'zustand/middleware';
import { INIT_ITEMS } from '../data/activities';

export const useAppStore = create(
  persist(
    (set) => ({
      // ── Panels ──────────────────────────────────────────────────
      sidebarOpen:       true,
      panelOpen:         true,
      chatOpen:          false,
      memoryPanelOpen:   false,
      brainGraphOpen:    false,
      newsOpen:          false,
      integrationsOpen:  false,
      inboxOpen:         false,
      contactsOpen:      false,

      setSidebarOpen:      (v) => set({ sidebarOpen: v }),
      setPanelOpen:        (v) => set({ panelOpen: v }),
      setChatOpen:         (v) => set({ chatOpen: v }),
      setMemoryPanelOpen:  (v) => set({ memoryPanelOpen: v }),
      setBrainGraphOpen:   (v) => set({ brainGraphOpen: v }),
      setNewsOpen:         (v) => set({ newsOpen: v }),
      setIntegrationsOpen: (v) => set({ integrationsOpen: v }),
      setInboxOpen:        (v) => set({ inboxOpen: v }),
      setContactsOpen:     (v) => set({ contactsOpen: v }),

      toggleSidebar:      () => set(s => ({ sidebarOpen:      !s.sidebarOpen })),
      togglePanel:        () => set(s => ({ panelOpen:        !s.panelOpen })),
      toggleChat:         () => set(s => ({ chatOpen:         !s.chatOpen })),
      toggleMemoryPanel:  () => set(s => ({ memoryPanelOpen:  !s.memoryPanelOpen })),
      toggleBrainGraph:   () => set(s => ({ brainGraphOpen:   !s.brainGraphOpen })),
      toggleNews:         () => set(s => ({ newsOpen:         !s.newsOpen })),
      toggleIntegrations: () => set(s => ({ integrationsOpen: !s.integrationsOpen })),
      toggleInbox:        () => set(s => ({ inboxOpen:        !s.inboxOpen })),
      toggleContacts:     () => set(s => ({ contactsOpen:     !s.contactsOpen })),

      // ── Navigation ──────────────────────────────────────────────
      activeNav:    'Dashboard',
      setActiveNav: (nav) => set({ activeNav: nav }),

      // ── Activity feed ────────────────────────────────────────────
      items:    INIT_ITEMS,
      latestId: null,
      addItem:     (item) => set(s => ({ items: [...s.items, item], latestId: item.id })),
      clearLatest: ()     => set({ latestId: null }),

      // ── Floating response cards ──────────────────────────────────
      cards:       [],
      addCard:     (card) => set(s => ({ cards: [...s.cards.slice(-2), card] })),
      removeCard:  (id)   => set(s => ({ cards: s.cards.filter(c => c.id !== id) })),
      clearCards:  ()     => set({ cards: [] }),

      // ── LLM source indicator ─────────────────────────────────────
      llmSource: null,
      setLlmSource: (src) => set({ llmSource: src }),

      // ── Dashboard AI context (set by active dashboard page) ──────
      dashboardAiContext: '',
      setDashboardAiContext: (ctx) => set({ dashboardAiContext: ctx }),

      // ── Organiser context (Phase D.4.6D) — current board/item the
      // operator has open, published by app/organiser/page.tsx's own
      // effect (mirrors dashboardAiContext's set-on-change/clear-on-
      // unmount pattern above) and read by hooks/useHelena.js's
      // sendMessage. IDs ONLY, never names — the server
      // (resolveHelenaOrganiserContext) resolves canonical names itself
      // and never trusts a client-supplied one. Deliberately NOT listed
      // in the persist() partialize below: unlike activeModule, this must
      // never survive a reload/new tab — it is only ever valid for as
      // long as the Organiser page that set it stays mounted.
      organiserContext: null, // { boardId?: string, itemId?: string } | null
      setOrganiserContext: (ctx) => set({ organiserContext: ctx }),

      // ── Helena message trigger (fire a message from outside the hook) ──
      helenaTrigger: null,
      fireHelena: (msg) => set({ helenaTrigger: msg }),
      clearHelenaTrigger: () => set({ helenaTrigger: null }),

      // ── Orb alert state (set when HLNA detects an anomaly) ──────
      orbAlert: false,
      setOrbAlert: (v) => set({ orbAlert: v }),
      clearOrbAlert: () => set({ orbAlert: false }),

      // ── Notification badge ───────────────────────────────────────
      notificationCount: 3,

      // ── Active department (context switcher) ─────────────────────
      // activeDepartment's own default ('waste') is left as-is — it feeds
      // BrainBase.jsx's mock department-briefing UI (MorningBriefing/
      // RecommendedActions/CommandSuggestions/ActivityPanel all call
      // getDeptConfig(activeDepartment) with no null-safety), so changing
      // it would risk breaking that dashboard rendering. departmentSelected
      // instead distinguishes "the user genuinely picked a department via
      // LeftSidebar" from "nobody has touched this yet" — see Phase C.2B.2:
      // hooks/useHelena.js only forwards `department` to /api/chat when
      // this is true, so the AI is never told "the operator is viewing the
      // Waste dashboard" for an org/session where no one ever selected one.
      activeDepartment:    'waste',
      departmentSelected:  false,
      setActiveDepartment: (key) => set({ activeDepartment: key, departmentSelected: true }),

      // ── Active module (module-aware HLNA context) ────────────────
      activeModule:    null,     // e.g. 'waste_recycling'
      setActiveModule: (key) => set({ activeModule: key }),

      enabledModules:    [],     // [{ key, name, industry }]
      setEnabledModules: (mods) => set({ enabledModules: mods }),

      // ── View mode (executive = headline KPIs; operational = detail) ─
      viewMode:    'executive',  // 'executive' | 'operational'
      setViewMode: (mode) => set({ viewMode: mode }),

      // ── Last upload timestamp (triggers briefing refresh) ────────
      lastUpload:    null,
      setLastUpload: (ts) => set({ lastUpload: ts }),
    }),
    {
      name: 'brainbase-state',
      // Only persist UI layout preferences — not ephemeral runtime state
      partialize: (state) => ({
        sidebarOpen:        state.sidebarOpen,
        panelOpen:          state.panelOpen,
        activeNav:          state.activeNav,
        activeModule:       state.activeModule,
        activeDepartment:   state.activeDepartment,
        viewMode:           state.viewMode,
      }),
    }
  )
);
